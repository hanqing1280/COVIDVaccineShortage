function h=mcmcpredplotwithcolor(out,colorlight,colordark,data,adddata)
...
for i=1:nbatch
  ...
  for j=1:ny
    %dimc = color(colorlight,:);%[0.9 0.9 0.9]; % dimmest (lightest) color
    %if ny>1;subplot(ny,1,j);end
    %if ~isempty(out.obslims)
    %  fillyy(time,out.obslims{i}{j}(1,:),out.obslims{i}{j}(3,:),dimc);
    %  hold on
    %  dimc = color(colorlight,:)./9*8; %[0.8,0.8,0.8];
    %end
    dimc = color(colorlight,:); %[0.8,0.8,0.8];
    fillyy(time,plimi{j}(1,:),plimi{j}(2*nn-1,:),dimc);
    hold on
    %for k=2:(nn-1)
    %  fillyy(time,plimi{j}(k,:),plimi{j}(2*nn-k,:),dimc.*0.9.^(k-1));
    %end
    plot(time,plimi{j}(nn,:),'-','Color',color(colordark,:),'LineWidth',2);
    if adddata
      plot(datai.ydata(:,1), datai.ydata(:,j+1),'o','Color',color(colordark,:),'LineWidth',2);
    end
    hold off
    if nbatch > 1
